    ...
    for (int site = 0; site < V; ++site) {
        for (int mu = 0; mu < 2; ++mu) {
            ...
            if (mu == 0) {
                ...
                double delta_old = field.get_link(x_m1, 0) + field.get_link(x_p0_m1, 1) - field.get_link(x_m1, 1);
                ...
                double delta_p1 = field.get_link(site, 0) + field.get_link(x_p0, 1) - field.get_link(x_p1, 0);
                
                // Term 2 from P(x-0) = Delta_p2 + theta_1(x)
                int x_m0 = geo.get_neighbor(site, 0, neg);
                int x_m0_p1 = geo.get_neighbor(x_m0, 1, pos);
                
                double delta_p2 = field.get_link(x_m0, 0) - field.get_link(x_m0_p1, 0) - field.get_link(x_m0, 1);
